...
export default function HomeFolders() {
	const { createFirstFolder, firstFolderDescription, folders, moreItems } = useLanguage(state => state.t)
	return (
		<section>
			<FolderCollection
				limit={10}
				...
				emptyTitle={createFirstFolder}
				emptyDescription={firstFolderDescription}
				header={total => (
					<h2 className='text-md text-foreground font-medium mb-4'>
						...
						>
						{folders} {total > 10 && moreItems.replace('{count}', String(total - 10))}
						</Link>
					</h2>
				)}
			/>
